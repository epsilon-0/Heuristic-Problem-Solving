...
public:
  EdmondsKarp(vector<vector<int>> graph, vector<vector<double>> capacities,
              int source, int sink) {
    this->graph = graph;
  ...
  vector<vector<int>> getFlowGraph() {
    vector<vector<int>> flowGraph(capacities.size(),
                                  vector<int>(capacities.size(), 0));
