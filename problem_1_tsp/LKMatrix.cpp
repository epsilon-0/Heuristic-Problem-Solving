...
void LKMatrix::LKMove(int tourStart) {
  ...
  do {
    ...
    for (int possibleNextV = tour[fromV];
         nextV == -1 && possibleNextV != tourStart;
         possibleNextV = tour[possibleNextV]) {
      // cout << "Testing " << possibleNextV << endl;
      // cout << (broken_set.count(make_sorted_pair(fromV, possibleNextV)) == 0)
      // << endl; cout << (possibleNextV != fromV) << endl; cout << (g + g_local
      // > 0) << endl; cout << (joined_set.count(make_sorted_pair(possibleNextV,
      // tour[possibleNextV])) == 0) << endl;
      ...
      if (!(
              ...
              joined_set.count(
                  make_sorted_pair(lastPossibleNextV, possibleNextV)) == 0 &&
              tour[possibleNextV] != 0 && // not already joined to start
              possibleNextV !=
                  tour[fromV] // not the one already joined to fromV
              )) {
