...
int walksat ( int mf, float pr );        // the main algorithm which returns the number of unsatisfied clauses after it has terminated
...
// given a clause we need to flip a variable which maximizes number of clauses which get satisfied
int climb ( int cl ){
  int result;
  int best_lit = -1;
  int curr_true;
