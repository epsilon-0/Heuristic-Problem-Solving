...
bool PrgBodyNode::simplifyBody(ProgramBuilder& prg, uint32 bodyId, std::pair<uint32, uint32>& hashes, Preprocessor& pre, bool strong) {
	...
	if (dirty && posS) {
		...
		// restore pos | neg partition
		// find last pos subgoal - must be there otherwise posS == 0
		uint32 j = dirty;
		while (goals_[--j].sign() == true) { ; }
		for (uint32 i = 0; ; --j, ++i) {
			// find first neg subgoal
			while (i < j && goals_[i].sign() == false) { ++i; }
			if (i >= j) break;
			// restore order
			std::swap(goals_[i], goals_[j]);	
			if (hasWeights()) {
				std::swap(extra_.ext->weights[i], extra_.ext->weights[j]);
			}
		}
	}
...
/////////////////////////////////////////////////////////////////////////////////////////
ProgramBuilder::ProgramBuilder() : minimize_(0), incData_(0), graph_(0), erMode_(mode_native) { }
...
ProgramBuilder& ProgramBuilder::startProgram(SharedContext& ctx, PBADG* ufs, const EqOptions& eqOpts) {
	disposeProgram(true);
...
bool ProgramBuilder::prepareProgram(const Solver& solver) {
	assert(!frozen_);
	transformExtended();
...
bool ProgramBuilder::transformProgram(Solver& solver, bool checkSccs) {
	CycleChecker c(atoms_, bodies_, checkSccs);
	if (incData_) {
		for (VarVec::const_iterator it = incData_->unfreeze_.begin(), end = incData_->unfreeze_.end(); it != end; ++it) {
			atoms_.push_back(atoms_[*it]);
		}
	}
	ctx_->startAddConstraints(solver);
	SatPreprocessor* p = 0;
	...
	if (checkSccs) stats.sccs = c.sccs();
	if (ret && c.hasCycles()) {
...
bool ProgramBuilder::endProgram(Solver& solver) {
	if (frozen_) {
		return cloneProgram(*ctx_, solver);
	}
	return prepareProgram(solver)
		&& transformProgram(solver, graph_ != 0);
}

bool ProgramBuilder::cloneProgram(SharedContext& ctx, Solver& solver) {
	assert(frozen_);
	if (&ctx == ctx_) {
		assert(ctx_->master() != &solver);
		return ctx_->attach(solver);
	}
	...
	ctx_   = &ctx;
	bool r = transformProgram(solver, false);
	ctx_    = t;
	return r;
}
...
bool ProgramBuilder::addConstraints(Solver& s, CycleChecker& c) {
	...
	const bool freezeAtoms = incData_ && s.sharedContext()->satPrepro.get() != 0;
