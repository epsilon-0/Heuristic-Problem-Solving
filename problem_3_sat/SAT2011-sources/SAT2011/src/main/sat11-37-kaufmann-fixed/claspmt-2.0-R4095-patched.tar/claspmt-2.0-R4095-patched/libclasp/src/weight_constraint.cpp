...
struct Shared_WeightConstraintLits {
	Shared_WeightConstraintLits(uint32 size, bool hasWeight) : shared_lits(size, true, hasWeight) {
		shared_counter = 1; 
	}
...
Shared_WeightConstraintLits* toSharedLits(WeightConstraintLits* x) {
...
#define OFFSETOF(S, x)  (size_t)&reinterpret_cast<const volatile unsigned char&>((((S*)0)->*&S::x))
	return reinterpret_cast<Shared_WeightConstraintLits*>(
		reinterpret_cast<unsigned char*>(x) - OFFSETOF(Shared_WeightConstraintLits, shared_lits)
	);
#undef OFFSETOF 
}
...
// WeightConstraint
/////////////////////////////////////////////////////////////////////////////////////////
bool WeightConstraint::newWeightConstraint(Solver& s, Literal con, WeightLitVec& lits, weight_t bound, Constraint** out) {
	assert(s.decisionLevel() == 0 && s.sharedContext()->unique() && s.sharedContext()->master() == &s);
	if (out) *out = 0;
	...
	Constraint* c = new (m) WeightConstraint(s, con, lits, bound, sumW);
	if (out) *out = c;
	s.add(c);
...
WeightConstraint::WeightConstraint(Solver& s, Literal W, const WeightLitVec& lits, uint32 bound, uint32 sumWeights) {
	const bool hasWeights = lits.front().second != lits.back().second;
	if (!s.sharedContext()->shareConstraints()) {
		...
		void* m = ::operator new(sizeof(mt::Shared_WeightConstraintLits) + ((1+lits.size()) << uint32(hasWeights))*sizeof(Literal));
		mt::Shared_WeightConstraintLits* x = new (m) mt::Shared_WeightConstraintLits(uint32(1+lits.size()), hasWeights);
		lits_ = &x->shared_lits;
	}
	Literal* heu  = new Literal[1+lits.size()];
	heu[0]        = W;
	Literal* p    = lits_->lits_;
	SharedContext* ctx  = s.sharedContext_unsafe();
	bool needData       = ctx && lits_->hasWeights();
