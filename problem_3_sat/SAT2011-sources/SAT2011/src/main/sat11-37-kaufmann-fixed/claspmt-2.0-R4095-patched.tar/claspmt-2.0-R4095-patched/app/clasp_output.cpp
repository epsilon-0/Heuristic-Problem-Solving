...
OutputFormat::OutputFormat(int verbosity) : curr_(0), next_(0), lpStats_(0), verbosity_(verbosity), quiet_(0) {
...
void OutputFormat::reportModel(const Solver& s, const Enumerator& en) {
	...
	if (modelQ() < print_no) {
		storeModel(s.assignment());
		if (modelQ() == print_all) {
			// fire & forget: just output the model
			Model m(curr_, s.sharedContext()->stats().vars+1); 
			curr_ = 0;
...
void OutputFormat::reportResult(const RunSummary& sol, const SolveStats** stats, size_t num) {
	...
	if (curr_ != 0) {
		...
		if (sol.consequences) {
			...
			Model m(curr_, sol.ctx.stats().vars+1);
...
void OutputFormat::storeModel(const Assignment& a) {
...
DefaultOutput::DefaultOutput(int v, const std::pair<int,int>& q, Format f, char ifs)
	: OutputFormat(v)
	, preStats_(0)
	, as_(0)
	, eom_(0)
...
void DefaultOutput::configureFormat(Format f, std::pair<int, int> q, int v, char ifs) {
	format_[cat_comment]   = "";
	format_[cat_value]     = "";
...
void DefaultOutput::reportProgress(Enumerator::ProgressType t, const Solver& s, uint64 maxCfl, uint32 maxL) {
	const char p[3] = {'R', 'D', 'M'};
	uint32 fixed    = s.decisionLevel() > 0 ? s.levelStart(1) : s.numAssignedVars();
	if (verbosity() >= 3 && --header_ == 0) {
		printf("%s----------------------------------------------------------------------------\n"
		       "%s[ID:T]     Vars           Constraints         State            Limits\n"
					 "%s       #free/#fixed   #problem/#learnt  #conflicts/ratio #conflict/#learnt\n"
					 "%s----------------------------------------------------------------------------\n", 
					 format_[cat_comment], format_[cat_comment], format_[cat_comment], format_[cat_comment]);
		header_ = 20;
	}
	comment(3, "[%u:%c]|%7u/%-7u|%8u/%-8u|%10"PRIu64"/%-6.3f|%8"PRId64"/%-8u|\n"
		, s.id()
		, p[t]
		, s.numFreeVars()
		, fixed
		...
		, maxCfl <= (UINT32_MAX) ? (int64)maxCfl:-1
		, maxL);
}

void DefaultOutput::reportState(int state, bool enter, double time) {
	if (enter && verbosity()) {
		if (state == ClaspFacade::state_read) {
	...
	else if (!enter && verbosity() > 1 && (state == ClaspFacade::state_read || state == ClaspFacade::state_preprocess)) {
		printf("%.3f", time);
		if (preStats_ != 0) {
			printf(" (ClRemoved: %u ClAdded: %u LitsStr: %u)", preStats_->clRemoved, preStats_->clAdded, preStats_->litsRemoved);
		}
		printf("\n"); fflush(stdout);
	}
}	

void DefaultOutput::reportPreProgress(char type, const SatPreprocessor& p, uint32 min, uint32 max) {
	printf("\r");
	if (type != '*' || min != max) {
		comment(2, "Preprocessing: %c: %8u/%-8u", type, min, max);
	}
	else {
		comment(2, "Preprocessing: ");
		preStats_ = &p.stats;
...
void DefaultOutput::printResult(const RunSummary& sol, const SolveStats** stats, size_t num) {
	...
	if (verbosity() > (0 - (num!=0))) {
		printf("\n");
		if      (sol.term == SIGALRM) { printKeyValue("TIME LIMIT", "%d\n", 1);  }
		else if (sol.term != 0)       { printKeyValue("INTERRUPTED", "%d\n", 1); }
		printKey("Models");
		if (!sol.complete) {
			char buf[64];
	...
	if (verbosity() >= 0 && num != 0) {
		printStats(sol.ctx.stats(), stats, num);
	}
}

void DefaultOutput::printStats(const ProblemStats& pr, const SolveStats** stats, std::size_t num) {
...
void DefaultOutput::printJumpStats(const SolveStats& st) const {
	if (st.jumps) {
		...
		w_ += 8;
		printf("\n");
		printKeyValue("Backtracks", "%-6"PRIu64"\n", st.conflicts-js.jumps);
		printKeyValue("Backjumps", "%-6"PRIu64, js.jumps);
		...
		printKeyValue("Average Jump Length", "%-6.1f", js.avgJumpLen());
		printf(" (Executed: %.1f)\n", js.avgJumpLenEx());
		printKeyValue("Average Bound Length", "%-6.1f\n", average(js.boundSum,js.bJumps));
		printKeyValue("Average Model Length", "%-6.1f\n", average(st.modLits,st.models));
		w_ -= 8;
	}
}

void DefaultOutput::printThreadStats(const SolveStats** stats, std::size_t num) const {
...
void DefaultOutput::printParallelStats(const SolveStats& stats, bool accu) const {
	if (stats.parallel) {
		...
		uint64 solved = ps.gps-uint64(ps.terminated);
		if (!accu) {
			printKeyValue("CPU Time",  "%.3fs\n", ps.cpuTime);
			printKeyValue("Models", "%"PRIu64"\n", stats.models);
		}
		printKeyValue("Distributed", "%"PRIu64"\n", ps.shared);
		printKeyValue("Splits", "%"PRIu64"\n", ps.splits);
		printKeyValue("Problems", "%-6"PRIu64, ps.gps);
		if (!accu) {
			printf(" (Solved: %"PRIu64" Average Length: %.2f) \n", solved, average(ps.gpLits, ps.gps));
		}
		else {
...
JsonOutput::JsonOutput(int v, const std::pair<int,int>& q) : OutputFormat(v), indent_(0), open_(""), hasModel_(false), hasWitness_(false) {
	setQuiet(q);
}
...
void JsonOutput::printValue(const char* s, const char* sep) {
	assert(s);
	const char* r = s;
	while (*r && *r != '"') { ++r; }
	if (!*r) {
		...
		std::string temp(s, 0, (r - s));
		do {
			if (*r != '=') { temp += *r; }
			else           { temp += "\\\""; }
		} while (*++r);
		printf("%s\"%s\"", sep, temp.c_str());
	}
}

void JsonOutput::printKeyValue(const char* k, const char* v) {
	printf("%s%-*s\"%s\": \"%s\"", open_, indent_, " ", k, v);
	open_ = ",\n";
}
...
void JsonOutput::reportPreProgress(char, const SharedContext&, uint32, uint32) { }
...
void JsonOutput::reportState(int state, bool enter, double) {
	...
	if (state == ClaspFacade::state_read && enter) {
...
void JsonOutput::startModel() {
	if (!hasWitness_) {
		startObject("Witnesseses", type_array);
		hasWitness_ = true;
	}
	if (hasModel_) {
		endObject();
...
void JsonOutput::printModel(const Model& m, const SymbolTable& index, const Enumerator&) {
	...
	if (index.used()) {
		for (SymbolTable::const_iterator it = index.begin(); it != index.end(); ++it) {
			if (m.value(it->second.lit.var()) == trueValue(it->second.lit) && !it->second.name.empty()) {
				printValue(it->second.name.c_str(), sep);
				sep = ", ";
			}	
		}
	}
...
void JsonOutput::printResult(const RunSummary& sol, const SolveStats** stats, std::size_t num) {
	...
	const Enumerator& en = *sol.ctx.enumerator();
	startObject("Stats");
	if      (sol.term == SIGALRM) { printKeyValue("TIME LIMIT", uint32(1));  }
	else if (sol.term != 0)       { printKeyValue("INTERRUPTED", uint32(1)); }
	...
	if (num > 0) {
		...
		printKeyValue("Other", p.constraints_ternary);
...
void JsonOutput::printLemmaStats(const SolveStats& st) {
	...
	printKeyValue("Other", sum - st.binary - st.ternary);
...
void JsonOutput::printJumpStats(const SolveStats& st) {
	if (st.jumps) {
		const JumpStats& js = *st.jumps;
		startObject("Jumps");
		printKeyValue("Backtracks", st.conflicts-js.jumps);
		printKeyValue("Backjumps", js.jumps);
		...
		printKeyValue("AvgJump", js.avgJumpLen());
		printKeyValue("AvgJumpEx", js.avgJumpLenEx());
		printKeyValue("AvgBound", average(js.boundSum,js.bJumps));
		printKeyValue("AvgModel", average(st.modLits,st.models));
		endObject();
	}
}

void JsonOutput::printThreadStats(const SolveStats** stats, std::size_t num) {
	if (stats[0]->parallel) {
		startObject("Parallel");
		const ParallelStats& ps = *stats[0]->parallel;
		printKeyValue("Distributed", ps.shared);
...
#if defined(WITH_CLASPRE_SUPPORT)
...
void Options::initOptions(ProgramOptions::OptionContext& root) {
	...
	pre.addOptions()
		("list-features"   , flag(listFeatures),"Print feature names (in --features=C1 format/order)")
...
bool Options::mapFormat(const std::string& s, int& format) {
	...
	if (s[0] == 'V' || s[0] == 'v') {
		...
		switch (s[1]) {
			case '1': format = features_compact_1; break;
			case '2': format = features_compact_2; break;
			case '3': format = features_compact_3; break;
...
public:
	...
	void reportProgress(Enumerator::ProgressType t, const Solver& s, uint64 maxCfl, uint32 maxL);
...
void ClaspreOutput::reportProgress(Enumerator::ProgressType t, const Solver& s, uint64 maxC, uint32 maxL) {
	if (t != Enumerator::progress_restart) return;
	...
	if (format_ == Options::features_verbose) {
		...
		if (s.stats.jumps) {
			printf("%-22s: %.3f\n", "Average Jump Length", s.stats.jumps->avgJumpLen());
		}
		printf("=================================\n\n");
	}
	else {
...
void ClaspreOutput::printResult(const RunSummary& sol, const SolveStats** stats, std::size_t num) {
	...
	if (format_ == Options::features_verbose) {
		...
		if (starts_ == 0) {
			Solver empty;
			reportProgress(Enumerator::progress_restart, empty, 0, 0);
		}
		...
		if (num > 0) {
			...
			temp << std::fixed << "," << st.models << "," << st.choices << "," << st.conflicts << "," << st.restarts << "," << st.deleted << "," 
					 << (st.conflicts-js.jumps) << "," << js.jumps << "," << js.bJumps << ","
					 << js.jumpSum << "," << (js.jumpSum - js.boundSum) << "," << percent(js.jumpSum - js.boundSum, js.jumpSum) << ","
					 << js.maxJump << "," << js.maxJumpEx << "," << js.maxBound << "," << js.avgJumpLen() << "," << js.avgJumpLenEx() << ","
					 << average(js.boundSum,js.bJumps) << "," << average(st.modLits,st.models) << ","
...
OutputFormat* Options::createOutput(int v, std::pair<int,int> q, DefaultOutput::Format f) { 
...
void Options::printFeatures() const {}
OutputFormat* Options::createOutput(int, std::pair<int,int>, DefaultOutput::Format) { return 0; }
}
#endif
