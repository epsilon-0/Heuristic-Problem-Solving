...
bool Solver::addUnary(Literal p, ConstraintType t) {
	...
	if (t != Constraint_t::static_constraint && shared_->learnBinary(p, negLit(0))) {
		stats.addLearnt(1, t);
		shared_->distributeShared(*this, p, negLit(0), negLit(0), t);	
	}
	return ret;
}
...
bool Solver::addBinary(Literal p, Literal q, ConstraintType t) {
	...
	if (shared_->unique()) {
		...
		stats.addLearnt(2, t);
		shared_->distributeShared(*this, p, q, negLit(0), t);
	}
	return true;
}
...
bool Solver::addTernary(Literal p, Literal q, Literal r, ConstraintType t) {
	...
	if (shared_->unique()) {
		...
		stats.addLearnt(3, t);
		shared_->distributeShared(*this, p, q, r, t);
	}
	return true;
}
...
bool ImplicationList::propagate(Solver& s, Literal p) const {
	...
	for (SWL::const_right_iterator it = right_begin(), end = right_end(); it != end; ++it) {
		q = it->first;
		r = it->second;
		if (s.isTrue(q) || s.isTrue(r)) continue;
		if (s.isFalse(r)) {
			if (!s.force(q, Antecedent(p, ~r))) return false;
		}
		else if (s.isFalse(q) && !s.force(r, Antecedent(p, ~q))) {
			return false;
		}
	}
#ifndef DISABLE_MULTI_THREADING
	if (Block* x = learnt()) {
		// BCP on learnt binary/ternary clauses
		do {
			for (const Literal* it = x->data, *end = x->data+x->size; it != end;) {
				if (s.isTrue(q = *it++)) {
					...
					if (!s.force(q, Antecedent(p))) return false;
				}
				else if (!s.isTrue(r = *it++)) {
					if (s.isFalse(r)) {
						if (!s.force(q, Antecedent(p, ~r))) return false;
					}
					else if (s.isFalse(q) && !s.force(r, Antecedent(p, ~q))) {
...
bool Solver::unitPropagate() {
	...
	Literal p, q, r;
	uint32 idx, ignore;
	const SharedContext::ShortClauseImplications& btig = shared_->shortClauseImplications();
	while ( !assign_.qEmpty() ) {
		p             = assign_.qPop();
		idx           = p.index();
		WatchList& wl = watches_[idx];
		// first: short clause BCP
		if (!btig[idx].propagate(*this, p)) {
			return false;
		}
		// second: clause BCP
		if (wl.left_size() != 0) {
...
uint32 Solver::estimateBCP(const Literal& p, int rd) const {
	...
	do {
		...
		const SWL& xList = btig[x.index()];
		for (SWL::const_left_iterator it = xList.left_begin(), end = xList.left_end(); it != end; ++it) {
			Literal y = *it;
...
bool Solver::ccReverseArc(const Literal p, uint32 maxLevel, uint32 maxNew, Antecedent& out) {
	...
	if ( (out = btig[p.index()].getReverseReason(*this, maxLevel)).isNull() ) {
