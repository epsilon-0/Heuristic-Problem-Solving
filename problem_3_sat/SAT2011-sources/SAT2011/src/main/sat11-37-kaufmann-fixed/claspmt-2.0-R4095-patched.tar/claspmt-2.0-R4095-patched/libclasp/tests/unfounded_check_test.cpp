...
public:
	UnfoundedCheckTest() : graph(0), ufs(0) { }
	void setUp() { 
		graph = new Clasp::mt::Shared_DependencyGraph(); 
	...
	void testOnlyOneSourceUnfoundedIfMinus() {
		...
		CPPUNIT_ASSERT(oldC+1 == ctx.numConstraints());
	}
	
	void testWithSimpleCardinalityConstraint() {
		builder.startProgram(ctx, graph.get())
	...
	void testNtExtendedFalse() {
		...
		// x :- not z.
		// y :- not z.
		builder.startProgram(ctx, graph.get(), ProgramBuilder::EqOptions().disable())
	...
	void testDependentExtReason() {
		...
		const Clasp::mt::Shared_DependencyGraph::AtomNode& a = *graph->getAtomByLit(ctx.symTab()[1].lit).node;
		Literal x = graph->getBodyNode(a.bodies()[1]).lit;
		...
		Literal extBody = graph->getBodyNode(a.bodies()[0]).lit;
		LitVec r;
		solver.reason(~ctx.symTab()[1].lit, r);
	...
	void testIncrementalUfs() {
		...
		;
		CPPUNIT_ASSERT_EQUAL(true, builder.endProgram(solver));
		CPPUNIT_ASSERT(10u == graph->nodes());
		CPPUNIT_ASSERT(1 == builder.stats.sccs);
		ctx.endInit();
...
private:
	Solver solver;
	SharedContext ctx;
	SingleOwnerPtr<Clasp::mt::Shared_DependencyGraph> graph;
	SingleOwnerPtr<DefaultUnfoundedCheck> ufs;
	ProgramBuilder builder;
	void attachUfs() {
		ufs->attachTo(solver, graph.get());
		ufs.release();
		ctx.endInit();
	}
