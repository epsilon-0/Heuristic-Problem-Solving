...
public:
   ...
   * All literals except the one passed to startAsserting() must be
	 ...
	 * \param extra Initial extra information (e.g. type) for the new clause
	 *
	 ...
	 */
	static Result create(Solver& s, LitVec& lits, uint32 flags = 0, const ClauseExtra& extra = ClauseExtra());
	 ...
	 *
	 * \note 
	 *   The function does not add local representations for implicit clauses (i.e. size <= 3) unless
	 *   modeFlags contains clause_explicit. In that case, an explicit representation is created.
	 *   To create an implicit representation, use ClauseCreator::createClause().
	 *
	 * \note
	 ...
	 *     bool r = true;
	 *     while (!F.empty() && r) {
	 *       mt::SharedLiterals* C = f.pop();
	 *       r = integrate(s, C, ...).ok;
	 *     }
	 *     return r;
	 *   \endcode
	 */
	static Result integrate(Solver& s, mt::SharedLiterals* clause, uint32 modeFlags, const ClauseExtra& e);
	/*!
	 * \overload Result integrate(Solver& s, mt::SharedLiterals* clause, uint32 modeFlags)
	 */
	static Result integrate(Solver& s, mt::SharedLiterals* clause, uint32 modeFlags);
	//@}
private:
	void init(ConstraintType t);
	struct Watches;
	static void   initWatches(Solver& s, LitVec& lits);
	static ClauseHead* newProblemClause(Solver& s, LitVec& lits, const ClauseExtra& e, uint32 flags);
	static ClauseHead* newLearntClause(Solver& s,  LitVec& lits, const ClauseExtra& e, uint32 flags);
	Solver*         solver_;    // solver in which new clauses are stored
	LitVec          literals_;  // literals of the new clause
	ClauseExtra     extra_;     // extra info 
...
public:
	...
	static ClauseHead*  newClause(Solver& s, const LitVec& lits) {
		return newClause(s, &lits[0], static_cast<uint32>(lits.size()), ClauseExtra());
	}
	static ClauseHead*  newClause(Solver& s, const Literal* lits, uint32 size, const ClauseExtra& extra);
	 ...
	 * \note The clause must be destroyed using Clause::destroy
	 */ 
	static ClauseHead*  newLearntClause(Solver& s, const LitVec& lits, const ClauseExtra& extra) {
	 ...
	 */ 
	static ClauseHead*  newContractedClause(Solver& s, const LitVec& lits, const ClauseExtra& extra, LitVec::size_type tail, bool extend);
	 ...
	 */
	static ClauseHead*  newShortClause(Solver& s, void* mem, const Literal* lits, uint32 size, const ClauseExtra& extra);
	...
	bool     contracted()           const { return (data_.extra[0] & 3u) == 3u; }
	bool     isSmall()              const { return (data_.extra[0] & 1u) == 0u; }
private:
	Clause(Solver& s, const ClauseExtra& extra, const Literal* lits, uint32 size, uint32 tail, bool extend);
	Clause() : ClauseHead(ClauseExtra()) {}
	static void* alloc(uint32 lits);
	void         undoLevel(Solver& s);
	PropResult   propagateTail(Solver& s, Literal, uint32 pos);
	typedef std::pair<Literal*, Literal*> LitRange;
	Literal*       longEnd()   { return head_+(data_.extra[0]>>2); }
	Literal*       smallEnd()  { return smallLits() + !isSentinel(*smallLits()) + !isSentinel(smallLits()[1]); }
	Literal*       smallLits() { return reinterpret_cast<Literal*>(data_.extra); }
	const Literal* smallLits() const { return reinterpret_cast<const Literal*>(data_.extra); }
