...
#define CLASP_VERSION "2.0.0-RC2"
#endif
...
//! Local options holding for one solver instance
class LocalOptions {
public:
...
//! Options for configuring solving threads
class ThreadOptions {
public:
	ThreadOptions();
	~ThreadOptions();
	bool   validate(std::string& err);
	void   setThreads(uint32 num);
	void   applyHeuristic();
	uint32    numThreads() const { return numThreads_; }
	LocalOptions* master() const { return master_; }
	LocalOptions* threadConfig(uint32 num) const {
		assert(num < numThreads_);
		if (num == 0) return master_;
		return &threads_[num-1];
	}
	void reset();
...
private:
	ThreadOptions(const ThreadOptions&);
	ThreadOptions& operator=(const ThreadOptions&);
	uint32     numThreads_; // number of solving threads
	LocalOptions* master_;  // master - always present
	LocalOptions* threads_; // configurations for additional solving threads
};

//! Global options holding for all solver instances
...
public:
	...
	GlobalOptions();
	ProgramBuilder* createApi(Clasp::mt::Shared_DependencyGraph** out);
	...
	struct Optimize {     /**< Optimization options */
		Optimize() : hierarch(0), heu(0), no(false), all(false) {}
...
private:
	...
	typedef SingleOwnerPtr<Clasp::mt::Shared_DependencyGraph> DG;
	// -------------------------------------------------------------------------------------------  
	// Status information
	...
	void   validateWeak();
	void   validateWeak(ThreadOptions& cfg);
	...
	SolveAlgorithm* createSolveObject(ClaspConfig& config) const;
	// -------------------------------------------------------------------------------------------
	ClaspConfig*           config_;
