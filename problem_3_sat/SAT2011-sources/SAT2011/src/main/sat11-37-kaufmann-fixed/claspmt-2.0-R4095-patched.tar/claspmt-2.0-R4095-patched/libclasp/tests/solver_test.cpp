...
struct TestingConstraint : public LearntConstraint {
	...
	bool isSatisfied(const Solver&, LitVec&) { return true; }
...
public:
	...
	void testCmpScores() {
		LearntConstraint::Activity a1 = makeActivity(100, 5);
		LearntConstraint::Activity a2 = makeActivity(90, 3);
		s.strategies().reduceScore    = SolverStrategies::score_act;
		CPPUNIT_ASSERT(s.strategies().compareScore(a1, a2) > 0);
		s.strategies().reduceScore    = SolverStrategies::score_lbd;
		CPPUNIT_ASSERT(s.strategies().compareScore(a1, a2) < 0);
		s.strategies().reduceScore    = SolverStrategies::score_both;
		CPPUNIT_ASSERT(s.strategies().compareScore(a1, a2) > 0);
	}
	...
	void testInitSavedValue() {
		Var v1 = ctx.addVar(Var_t::atom_var);
		Var v2 = ctx.addVar(Var_t::body_var);
		ctx.startAddConstraints(s); ctx.endInit();
	...
	void testStrengthenConditional() {
		...
		ClauseCreator cc(&s);
		Literal tag = ctx.tagLiteral();
		cc.start(Constraint_t::learnt_conflict).add(posLit(a)).add(posLit(b)).add(posLit(c)).add(~tag);
		Constraint* newCon = cc.end().local;
		CPPUNIT_ASSERT(s.numLearntConstraints() == 1);
	...
	void testIncrementalSolve() {
		...
		struct IncInput : Input {
			...
			void getAssumptions(LitVec& a) {
				if (assume[step-1] != 0) {
					a.push_back( ~solver->sharedContext()->symTab().find(assume[step-1])->lit );
				}
			}
			bool read(Solver& s, ApiPtr p, int) {
				if (step == 0) {
	...
	void testLearntShort() {
		...
		Literal c = posLit(ctx.addVar( Var_t::atom_var ));
		Literal d = posLit(ctx.addVar( Var_t::atom_var ));
		ctx.enableConstraintSharing();
		ctx.startAddConstraints(s);
		ctx.addBinary(c, d);
		ctx.endInit(2);
		SharedContext* other = &ctx;
	...
	void testLearntShortAreDistributed() {
		...
		struct Dummy : public Distributor {
			Dummy() : Distributor(2, 3, 32), unary(0), binary(0), ternary(0) {}
			SharedLiterals* doPublish(const Solver& source, const Literal* lits, uint32 size, const ClauseExtra& extra) {
				unary   += size == 1;
				binary  += size == 2;
				ternary += size == 3;
				shared.push_back(SharedLiterals::newShareable(lits, size, extra.type(), 2));
				return shared.back();
			}
			uint32 receive(const Solver& in, SharedLiterals** out, uint32 num) { 
				uint32 r = 0;
		...
		ctx.enableLearntSharing(dummy = new Dummy());
		ctx.startAddConstraints(s);
		ctx.endInit(2);
		SharedContext* other = &ctx;
		LitVec lits; lits.resize(2);
		lits[0] = a; lits[1] = b;
		ClauseCreator::create(s, lits, 0, ClauseExtra().setType(Constraint_t::learnt_conflict));
		lits.resize(3);
		lits[0] = ~a; lits[1] = ~b; lits[2] = ~c;
		ClauseCreator::create(s, lits,0, ClauseExtra().setType(Constraint_t::learnt_loop));
		lits.resize(1);
		lits[0] = d;
		ClauseCreator::create(s, lits, 0, ClauseExtra().setType(Constraint_t::learnt_conflict));
