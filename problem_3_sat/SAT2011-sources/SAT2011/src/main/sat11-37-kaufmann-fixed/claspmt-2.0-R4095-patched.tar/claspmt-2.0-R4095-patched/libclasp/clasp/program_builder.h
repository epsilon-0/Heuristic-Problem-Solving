...
namespace mt { class	Shared_DependencyGraph; }
...
public:
	...
	struct EqOptions {
		EqOptions() : iters(5), dfOrder(false), backprop(false) {}
		EqOptions& setIterations(int it)    { iters   = it;   return *this;}
		EqOptions& enableDfOrder()          { dfOrder = true; return *this;}
		EqOptions& enableBackpropagation()  { backprop= true; return *this;}
		EqOptions& disable()                { iters   = 0;    return *this;}
		int    iters;   /**< Number of iterations - 0 = disabled */
	 ...
	 * \param ctx    The context object in which the program builder should store the symbol table and variable information
	 * \param dg     The object to use for storing the positive body-atom dependency graph. If 0, dependencies are ignored
	 ...
	 */
	ProgramBuilder& startProgram(SharedContext& ctx, mt::Shared_DependencyGraph* dg, const EqOptions& eqOpts = EqOptions());
	 ...
	 *
	 * \note endProgram() attaches the given solver as master solver to the context object that was given
	 * in the call to startProgram().
	 *
	 ...
	 *
	 */
	bool endProgram(Solver& solver);

	//! Clones the program and adds it to the given solver
	/*
	 * \pre The program is currently frozen
	 * 
	 * If ctx is equal to the context object that was initially passed to startProgram(),
	 * cloneProgram() simply attaches solver to context.
	 * Otherwise the program is cloned and solver is used as the master solver in the
	 * given context.
	 */
	bool cloneProgram(SharedContext& ctx, Solver& solver);
...
private:
	...
	// ------------------------------------------------------------------------
	// Nogood creation
	bool prepareProgram(const Solver& s);
	bool transformProgram(Solver& s, bool checkSccs);
	...
	typedef mt::Shared_DependencyGraph PBADG;
	PBADG*           graph_;    // positive dependency graph if program is not tight
