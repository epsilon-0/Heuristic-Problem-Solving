...
public:
	...
	void testTightProgram() { 
		builder.startProgram(*ctx, graph)
		...
		CPPUNIT_ASSERT_EQUAL(uint32(0), graph->nodes());
	}
	
	void testInitOrder() {
		builder.startProgram(*ctx, graph, ProgramBuilder::EqOptions().disable())
