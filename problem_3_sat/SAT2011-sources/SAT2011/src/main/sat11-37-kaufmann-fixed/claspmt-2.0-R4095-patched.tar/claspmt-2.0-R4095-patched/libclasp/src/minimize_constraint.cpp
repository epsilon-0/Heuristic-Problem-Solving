...
bool MinimizeConstraint::propagateImpl(Solver& s, PropMode m) {
	...
	for (; ok && !isSentinel(it->first); ++it, ++idx) {
		...
		if (!s.isFalse(it->first) || (impLevel < DL && s.level(it->first.var()) > impLevel)) {
			if (impLevel == DL || impLevel == (DL = s.undoUntil(impLevel, true))) {
				ok = s.force(~it->first, this, undoPos);
			}
			else {
				ok = forceOutOfOrder(s, ~it->first, impLevel, undoPos);
			}
		}
	}
	return ok;
...
bool MinimizeConstraint::forceOutOfOrder(Solver& s, Literal p, uint32 impDL, uint32 undoPos) {
	uint32 old = UINT32_MAX;
	if (s.isTrue(p)) {
		const Antecedent& ante = s.reason(p);
		if (ante.type() == Antecedent::generic_constraint && ante.constraint() == this) {
			old = s.reasonData(p);
		}
		// p is currrently implied by some other constraint but
		// our new constraint is stronger
		s.setReason(p, this, undoPos);
	}
	return old == undoPos || s.addNewImplication(p, impDL, this, undoPos);
}
...
//
// For now we use a simple "swith-on-type" approach. 
...
void SharedMinimizeData::setStep(Optimum* o) {
	if (hierarch_ > 1) {
		...
		low_         = std::min(sum_[lev], wsum_t(0));
...
void MinimizeBuilder::prepare(Solver& s) {
	std::sort(lits_.begin(), lits_.end(), CmpByLit());
	LitVec::size_type j = 0, k;
	SharedContext* ctx  = s.sharedContext_unsafe();
	Weight* w           = 0;
	for (LitVec::size_type i = 0, end = lits_.size(); i != end;) {
		w    = lits_[i].second;
...
MinimizeBuilder::SharedData* MinimizeBuilder::build(Solver& s, Literal ma) {
	assert(!s.hasConflict());
	if (!s.propagate()) return 0;
	if (!ready_) {
		prepare(s);
		addAssumption(ma);
		ready_ = true;
...
MinimizeConstraint* MinimizeBuilder::buildAndAttach(Solver& s, MinimizeMode mode, int heuristic) {
	SharedData* d         = build(s);
	d->setMode(mode, false);
	MinimizeConstraint* m = attach(s, d, heuristic);
	m->ownsShared_ = true;
	return m;
}
