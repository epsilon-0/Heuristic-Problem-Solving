...
bool solve(Solver& s, const SolveParams& p) {
	return SimpleSolve().solve(*s.sharedContext_unsafe(), p, LitVec());
}

bool solve(Solver& s, const SolveParams& p, const LitVec& assumptions) {
	return SimpleSolve().solve(*s.sharedContext_unsafe(), p, assumptions);
}
...
bool SimpleSolve::backtrackFromModel(Solver& s) { 
	return (enumRes_ = s.sharedContext()->enumerator()->backtrackFromModel(s))
		== Enumerator::enumerate_continue;
}
bool SimpleSolve::doSolve(Solver& s, const SolveParams& p, const LitVec& assume) {
	...
	for (enumRes_ = Enumerator::enumerate_continue; enumRes_ == Enumerator::enumerate_continue; ) {
		enumRes_ = Enumerator::enumerate_stop_complete;
		...
		if (enumerator->optimizeHierarchical() && enumRes_ != Enumerator::enumerate_stop_enough) {
			min = enumerator->constraint(s)->minimize();
			// restore previous optimum - this relaxes the constraint
			min->restoreOptimum();
			enumRes_ = res != value_free && enumerator->enumerated > 0 && min->shared_unsafe()->optimizeNext()
				? Enumerator::enumerate_continue
				: Enumerator::enumerate_stop_complete;
		}
	}
	bool more = res == value_free || s.decisionLevel() > s.rootLevel();
	// Finally, remove the assumptions again and restore
	// the solver to a usable state if possible.
	if (!s.clearAssumptions()) {
		more = false;
	}
	enumerator->reportResult(!more);
	return more;
}
