...
public:
	...
	void testClauseCtorAddsWatches() {
		...
		ClauseHead* cl2 = Clause::newLearntClause(solver, clLits, ClauseExtra().setType(Constraint_t::learnt_conflict));
		solver.add(cl2);
	...
	void testClauseTypes() {
		...
		LearntConstraint* cl2 = Clause::newLearntClause(solver, clLits, ClauseExtra().setType(Constraint_t::learnt_conflict));
		LearntConstraint* cl3 = Clause::newLearntClause(solver, clLits, ClauseExtra().setType(Constraint_t::learnt_loop));
	...
	void testClauseActivity() {
		...
		uint32 exp = 258;
		ClauseHead* cl1 = Clause::newLearntClause(solver, lit, ClauseExtra().setType(Constraint_t::learnt_conflict).setActivity(exp));
		ClauseHead* cl2 = Clause::newLearntClause(solver, lit, ClauseExtra().setType(Constraint_t::learnt_loop).setActivity(exp));
		solver.add(cl1);
		solver.add(cl2);
		while ( exp != 0 ) {
			CPPUNIT_ASSERT(cl1->activity().score() == cl2->activity().score() && cl1->activity().score() == exp);
			exp >>= 1;
	...
	void testReasonBumpsActivityIfLearnt() {
		...
		ctx.endInit(1);
		LearntConstraint* cl1 = Clause::newLearntClause(solver, clLits, ClauseExtra().setType(Constraint_t::learnt_conflict));
		...
		uint32 a = cl1->activity().score();
		LitVec r;
		solver.reason(clLits[3], r);
		CPPUNIT_ASSERT_EQUAL(a+1, cl1->activity().score());
	}
	...
	void testStrengthen() {
		...
		clLits.push_back(b2);
		clLits.push_back(b3);
		c = Clause::newLearntClause(solver, clLits, ClauseExtra());
	...
	void testStrengthenToUnary() {
		...
		clLits.push_back(b);
		clLits.push_back(~a);
		ClauseExtra extra; extra.tag().setType(Constraint_t::learnt_conflict);
	...
	void testStrengthenContracted() {
		solver.strategies().setCompressionStrategy(4);
		ctx.endInit(1);
	...
	void testNewContractedClause() {
		...
		ClauseHead* c = Clause::newContractedClause(solver, clLits, ClauseExtra().setType(Constraint_t::learnt_conflict), 3, false);
