	...
	if (project_ && ctx.symTab().used()) {
		...
		for (SymbolTable::const_iterator it = index.curBegin(); it != index.end(); ++it) {
			if (!it->second.name.empty()) {
				...
				if (it->second.name[0] != '_' && ctx.master()->value(v) == value_free) {
...
Enumerator::SolveConstraint* BacktrackEnumerator::doInit(SharedContext& ctx, uint32, bool start) {
	if (start) {
...
bool BacktrackEnumerator::doBacktrack(Solver& s, uint32 bl) {
	...
	if (!projectionEnabled() || bl <= btLevel) {
		...
		static_cast<BTConstraint*>(s.getEnumerationConstraint())->add(s, c);
...
/////////////////////////////////////////////////////////////////////////////////////////
// class BacktrackEnumerator::Constraint
/////////////////////////////////////////////////////////////////////////////////////////
void BacktrackEnumerator::BTConstraint::destroy(Solver* s, bool detach) {
	while (!nogoods.empty()) {
...
void BacktrackEnumerator::BTConstraint::add(Solver& s, LearntConstraint* c) {
...
void BacktrackEnumerator::BTConstraint::undoLevel(Solver& s) {
...
Constraint* BacktrackEnumerator::BTConstraint::cloneAttach(Solver& other) {
	BTConstraint* ret = new BTConstraint();
...
Enumerator::SolveConstraint* RecordEnumerator::doInit(SharedContext& ctx, uint32 t, bool start) {
	if (start) {
		ModelEnumerator::initContext(ctx);
		return 0;
	}
	...
	return new RecordConstraint(queue_);
}
...
bool RecordEnumerator::doBacktrack(Solver& s, uint32 bl) {
	...
	RecordConstraint* c  = static_cast<RecordConstraint*>(s.getEnumerationConstraint());
	return c->add(s, solution_.lits(), ClauseExtra().setType(Constraint_t::learnt_other));
}

bool RecordEnumerator::updateConstraint(Solver& s, bool) {
	RecordConstraint* con = static_cast<RecordConstraint*>(s.getEnumerationConstraint());
...
/////////////////////////////////////////////////////////////////////////////////////////
RecordEnumerator::RecordConstraint::RecordConstraint(SolutionQueue* q) : queue(q), id(0) {
	if (queue) { id = queue->addThread(); }
}
bool RecordEnumerator::RecordConstraint::simplify(Solver& s, bool) {
	s.simplifyDB(db);
	return false;
}
void RecordEnumerator::RecordConstraint::destroy(Solver* s, bool detach) {
	while (!db.empty()) {
...
Constraint* RecordEnumerator::RecordConstraint::cloneAttach(Solver& other) {
	RecordConstraint* ret = new RecordConstraint(queue);
	ret->setMinimize(cloneMini(other));
	return ret;
}

bool RecordEnumerator::RecordConstraint::add(Solver& s, LitVec& lits, const ClauseExtra& e) {
	ClauseCreator::Result ret;
	if (!queue) {
		ret = ClauseCreator::create(s, lits, ClauseCreator::clause_no_add, e);
		if (ret.local) { db.push_back(ret.local); }
	}
	else {
		using mt::SharedLiterals;
		...
		ret.ok = integrate(s, shared, flags()|ClauseCreator::clause_known_order);
	}
	return ret.ok || s.resolveConflict();
}

bool RecordEnumerator::RecordConstraint::integrateAll(Solver& s) {
	if (queue) {
		uint32 f = flags();
...
bool RecordEnumerator::RecordConstraint::integrate(Solver& s, SL* clause, uint32 f) {
	ClauseCreator::Result res = ClauseCreator::integrate(s, clause, f);
	if (res.local) {
		db.push_back(res.local);
	}
	return res.ok;
}
