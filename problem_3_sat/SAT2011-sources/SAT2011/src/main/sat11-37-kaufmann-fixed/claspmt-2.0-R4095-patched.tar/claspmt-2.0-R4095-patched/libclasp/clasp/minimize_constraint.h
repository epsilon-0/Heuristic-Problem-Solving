...
public:
	 ...
	 * \param s  A solver used to simplify minimize statements
	 * \param ma An assumption that should be used for tagging reasons
	 */
	SharedData*         build(Solver& s, Literal ma = posLit(0));
	 ...
	 *  d->setMode(m);
	 *  return attach(s, d, heuristic);
	 */
	MinimizeConstraint* buildAndAttach(Solver& s, MinimizeMode m = MinimizeMode_t::optimize, int heuristic = 0);
private:
	struct Weight {
...
protected:
	...
	bool      forceOutOfOrder(Solver& s, Literal p, uint32 impDL, uint32 undoPos);
