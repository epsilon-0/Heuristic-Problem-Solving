...
public:
	...
	SymbolTable() : lastSort_(map_type::size_type(-1)), lastStart_(0) { }
	~SymbolTable() { clear();  }
	...
	//! symbol table in use?
	bool   used() const { return lastSort_ != map_type::size_type(-1); }
	//! returns the number of symbols in this table
	uint32 size() const { return (uint32)map_.size(); }
	...
	const_iterator lower_bound(const_iterator start, key_type i) const {
		assert(lastSort_ == map_.size() && (start == map_.begin() || start == map_.end() || start->first <= i));
