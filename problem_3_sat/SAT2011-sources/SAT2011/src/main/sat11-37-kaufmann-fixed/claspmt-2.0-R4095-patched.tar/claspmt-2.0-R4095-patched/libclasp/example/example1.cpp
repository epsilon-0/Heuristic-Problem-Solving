...
void example1() {
	...
	// The Solver is the main class of clasp and hosts
	// the data and functions for CDNL answer set solving.
	// See solver.h for details
	Clasp::Solver         solver;	
	
	// Among other things, SharedContext contains the symbol table 
	// which stores the mapping between
	// atoms of the logic program and the propositional 
	// literals in the solver.
	Clasp::SharedContext  ctx;
	
	// startProgram must be called once before we can add atoms/rules
	Clasp::mt::Shared_DependencyGraph graph;
	api.startProgram(ctx, &graph);
	...
	if (graph.nodes() > 0) {
		...
		ufs->attachTo(solver, &graph); // register with solver and graph & transfer ownership
	}
