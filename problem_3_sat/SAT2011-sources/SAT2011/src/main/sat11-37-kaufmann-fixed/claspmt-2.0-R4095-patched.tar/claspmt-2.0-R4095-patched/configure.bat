...
IF %mt%==0 (
	SET CXXFLAGS=%CXXFLAGS% -DDISABLE_MULTI_THREADING
) ELSE (
	if [%TBB_INCLUDE%]==[] (
	...
	SET CXXFLAGS=%CXXFLAGS% -I %TBB_INCLUDE%
	SET LDFLAGS=%LDFLAGS% -ltbb -L %TBB_LIB% -Xlinker "--rpath=%TBB_LIB%"
	SET BUILDPATH=%BUILDPATH%_mt
)
IF %claspre%==1 (
	SET CXXFLAGS=%CXXFLAGS% -DWITH_CLASPRE_SUPPORT
)
IF %static%==1 (
