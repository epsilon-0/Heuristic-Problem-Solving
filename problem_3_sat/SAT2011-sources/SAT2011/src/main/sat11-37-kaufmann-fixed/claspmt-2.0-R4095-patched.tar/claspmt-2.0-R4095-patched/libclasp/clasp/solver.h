...
struct SolverStrategies {
	...
	int                 compareScore(const LearntConstraint::Activity& lhs, const LearntConstraint::Activity& rhs) const {
		switch (reduceScore) {
			...
			case score_both: 
			default: return ((int)lhs.scoreBoth()) - ((int)rhs.scoreBoth());
		}
		return 0;
	}
...
public:
	...
	/*!
	 * \note Modifying the shared object *is not* thread-safe!
	 */
	SharedContext* sharedContext_unsafe()      { return shared_; }
	...
	/*!
	 * \pre dl <= decisionLevel()
	 * \note if dl < decisionLevel(), p is actually assigned at decisionLevel() but
	 * the solver stores enough information to reassign p on each level x in [dl, decisionLevel())
	 */
	bool addNewImplication(Literal p, uint32 dl, const Antecedent& r, uint32 d = UINT32_MAX) {
...
private:
	...
	ClauseExtra       ccInfo_;      // temporary: stores information about conflict clause cc_
