...
// DIMACS PARSING
/////////////////////////////////////////////////////////////////////////////////////////
bool parseDimacsImpl(std::istream& prg, SharedContext& ctx, Solver& s, bool assertPure) {
	...
	for (;ret;) {
		...
		if (*in == 0) {
			...
			if (match(in, "p cnf", false)) {
				...
				ctx.startAddConstraints(s, std::min(numClauses, 10000));
...
class OPBParser {
public:
	OPBParser(std::istream& in, SharedContext& ctx, Solver& s, ObjectiveFunction& obj) 
		: source_(in)
		, ctx_(ctx)
		, solver_(s)
		, obj_(obj) {
...
void OPBParser::parseHeader() {
	...
	// Init solver
	ctx_.reserveVars(numVars_+1);
	for (int v = 1; v <= numVars_; ++v) {
		ctx_.addVar(Var_t::atom_var);
	}
	ctx_.startAddConstraints(solver_);
}
...
bool parseDimacs(std::istream& prg, SharedContext& ctx, Solver& s, bool assertPure) {
	return parseDimacsImpl(prg, ctx, s, assertPure);
}

bool parseOPB(std::istream& prg, SharedContext& ctx, Solver& s, ObjectiveFunction& objective) {
	OPBParser parser(prg, ctx, s, objective);
	return parser.parse();
}
...
Input::Format detectFormat(std::istream& in) {
	...
	msg[1]     = x != std::char_traits<char>::eof() ? (char)(unsigned char)x:'?';
	in ? throw ReadError(1, msg)
...
bool StreamInput::read(Solver& s, ApiPtr api, int numModels) {
	if (format_ == Input::SMODELS) {
		return parseLparse(prg_, *api.api);
	}
	else if (format_ == Input::DIMACS) {
		return parseDimacs(prg_, *api.ctx, s, numModels == 1);
	}
	else {
		return parseOPB(prg_, *api.ctx, s, func_);
	}
}
