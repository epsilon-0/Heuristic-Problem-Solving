...
public:
	...
	void testDuplicateRule() {
		builder.startProgram(ctx, ufs, ProgramBuilder::EqOptions().setIterations(1))
			.setAtomName(1, "a").setAtomName(2, "b")
			.startRule(CHOICERULE).addHead(2).endRule()  // {b}.
			.startRule().addHead(1).addToBody(2, true).endRule()  // a :- b.
			.startRule().addHead(1).addToBody(2, true).endRule()  // a :- b.
		;
		CPPUNIT_ASSERT_EQUAL(true, builder.endProgram(solver) && ctx.endInit());
	...
	void testMergeToSelfblocker() {
		...
		;
		CPPUNIT_ASSERT_EQUAL(false, builder.endProgram(solver) && ctx.endInit());
	}
	...
	void testAddOneFact() {
		...
		CPPUNIT_ASSERT_EQUAL(true, builder.endProgram(solver));
		CPPUNIT_ASSERT_EQUAL(0u, solver.numVars());
		builder.writeProgram(str);
	...
	void testDontAddUnsupportedNoEq() {
		...
		// c :- not d.
		// b :- a.
		builder.startProgram(ctx, ufs,ProgramBuilder::EqOptions().disable())
			.setAtomName(1, "a").setAtomName(2, "b").setAtomName(3, "c").setAtomName(4, "d")
			.startRule().addHead(1).addToBody(3, true).addToBody(2, true).endRule()
			.startRule().addHead(3).addToBody(4, false).endRule()
			.startRule().addHead(2).addToBody(1, true).endRule()		
	...
	void testDontAddUnsupportedExtNoEq() {
		...
		CPPUNIT_ASSERT_EQUAL(true, solver.isTrue(ctx.symTab()[1].lit));
		CPPUNIT_ASSERT_EQUAL(true, solver.isTrue(ctx.symTab()[2].lit));
	}
	...
	void testCloneShare() {
		// {a, b, c}.
		// d :- a, b, c. 
		builder.startProgram(ctx, ufs, ProgramBuilder::EqOptions().disable()) // no prepro
			.setAtomName(1, "a").setAtomName(2, "b").setAtomName(3, "c").setAtomName(4, "d")
			.startRule(CHOICERULE).addHead(1).addHead(2).addHead(3).endRule()
			.startRule().addHead(4).addToBody(1, true).addToBody(2, true).addToBody(3, true).endRule()
		;
		CPPUNIT_ASSERT_EQUAL(true, builder.endProgram(solver) && ctx.endInit(3));
		CPPUNIT_ASSERT_EQUAL( uint32(6), solver.numVars() );
		CPPUNIT_ASSERT_EQUAL( uint32(6), ctx.numConstraints() );

		Solver solver2;
		CPPUNIT_ASSERT_EQUAL(true, builder.endProgram(solver2));
		CPPUNIT_ASSERT(&ctx == solver2.sharedContext());
		CPPUNIT_ASSERT_EQUAL( uint32(6), solver2.numVars() );
		CPPUNIT_ASSERT_EQUAL( uint32(6), solver2.numConstraints() );

		Solver solver3;
		CPPUNIT_ASSERT_EQUAL(true, builder.cloneProgram(ctx, solver3));
		CPPUNIT_ASSERT(&ctx == solver3.sharedContext());
		CPPUNIT_ASSERT_EQUAL( uint32(6), solver3.numVars() );
		CPPUNIT_ASSERT_EQUAL( uint32(6), solver3.numConstraints() );

		CPPUNIT_ASSERT(!ctx.unique());
	}
	...
	void testCloneShareSymbols() {
		...
		CPPUNIT_ASSERT_EQUAL(true, builder.cloneProgram(ctx2, solver2) && ctx2.endInit());
		CPPUNIT_ASSERT_EQUAL( uint32(6), solver2.numVars() );
		CPPUNIT_ASSERT_EQUAL( uint32(6), solver2.numConstraints() );
		CPPUNIT_ASSERT(solver.sharedContext() != solver2.sharedContext());
		CPPUNIT_ASSERT(solver.sharedContext()->unique());
		CPPUNIT_ASSERT(solver2.sharedContext()->unique());
		CPPUNIT_ASSERT( &solver.sharedContext()->symTab() == &solver2.sharedContext()->symTab() );
	}
	void testCloneFull() {
		// {a, b, c}.
		// d :- a, b, c. 
	...
	void testSatBodyBug() {
		...
		;
		CPPUNIT_ASSERT_EQUAL(true, builder.endProgram(solver) && ctx.endInit());
		CPPUNIT_ASSERT_EQUAL(value_free, solver.value(ctx.symTab()[3].lit.var()));
	}
	...
	void testAddMinimizeConstraint() {
		...
		MinimizeConstraint* c1 = minBuilder.buildAndAttach(solver);
		SharedContext ctx2; Solver solver2;
		CPPUNIT_ASSERT_EQUAL(true, builder.cloneProgram(ctx2, solver2));
		MinimizeConstraint* c2 = minBuilder.buildAndAttach(solver2);

		CPPUNIT_ASSERT(c1 != 0 && c2 != 0 && c1 != c2);
		c1->destroy(&solver, true);
		c2->destroy(&solver2, true);
		builder.writeProgram(str);
	...
	void testEqOverComp() {
		...
		CPPUNIT_ASSERT(solver.numFreeVars() == 0 && solver.isTrue(ctx.symTab()[1].lit));
	}
	...
	void testEqOverBodiesOfDiffType() {
		...
		CPPUNIT_ASSERT(solver.isFalse(ctx.symTab()[3].lit));
	...
	void testNoBodyUnification() {
		...
		ufsCheck->attachTo(solver, ufs);
		CPPUNIT_ASSERT_EQUAL(true, ctx.endInit());
		solver.assume(~ctx.symTab()[2].lit);	// ~y
		solver.propagate();
		solver.assume(~ctx.symTab()[3].lit);	// ~z
		solver.propagate();
		CPPUNIT_ASSERT_EQUAL(true, solver.isFalse(ctx.symTab()[7].lit));
	}
	...
	void testNoEqAtomReplacement() {
		...
		;
		CPPUNIT_ASSERT_EQUAL(true, builder.endProgram(solver) && ctx.endInit());
		solver.assume(~ctx.symTab()[1].lit);	// ~x
		solver.propagate();
		solver.assume(~ctx.symTab()[2].lit);	// ~y
		solver.propagate();
		CPPUNIT_ASSERT_EQUAL(true, solver.isFalse(ctx.symTab()[3].lit));
	...
	void testAllBodiesSameLit() {
		...
		solver.assume(ctx.symTab()[1].lit) && solver.propagate();
		CPPUNIT_ASSERT(solver.value(ctx.symTab()[3].lit.var()) == value_free);
		solver.assume(~ctx.symTab()[3].lit) && solver.propagate();
		CPPUNIT_ASSERT(solver.numFreeVars() == 0 && solver.isTrue(ctx.symTab()[2].lit));
	}
	...
	void testFunnySelfblockerOverEqByTwo() {
		...
		;
		CPPUNIT_ASSERT_EQUAL(true, builder.endProgram(solver) && ctx.endInit());
		CPPUNIT_ASSERT(solver.isFalse(ctx.symTab()[8].lit));
		CPPUNIT_ASSERT(solver.isFalse(ctx.symTab()[9].lit));
	}
	...
	void testIncrementalFreeze() {
		...
		solver.propagate();
		CPPUNIT_ASSERT(solver.isTrue(ctx.symTab()[2].lit));
	}
	...
	void testIncrementalUnfreezeUnsupp() {
		...
		CPPUNIT_ASSERT_EQUAL(true, builder.endProgram(solver));
		ctx.endInit();;
		CPPUNIT_ASSERT(solver.isTrue(ctx.symTab()[1].lit));
		CPPUNIT_ASSERT(solver.isFalse(ctx.symTab()[2].lit));
		CPPUNIT_ASSERT(solver.isFalse(ctx.symTab()[3].lit));
	}
	...
	void testIncrementalCompute() {
		...
		;
		CPPUNIT_ASSERT_EQUAL(true, builder.endProgram(solver) && ctx.endInit());
		solver.assume(ctx.symTab()[2].lit);
		solver.propagate();
		CPPUNIT_ASSERT(solver.isFalse(ctx.symTab()[3].lit));
		CPPUNIT_ASSERT(solver.isFalse(ctx.symTab()[4].lit));
	}
	...
	void testBackprop() {
		builder.startProgram(ctx, ufs, ProgramBuilder::EqOptions().enableBackpropagation())
			.setAtomName(1, "a").setAtomName(2, "b").setAtomName(3, "c").setAtomName(4, "d")
...
private:
	...
	typedef Clasp::mt::Shared_DependencyGraph DG;
	DG* ufs;
	stringstream str;
};
