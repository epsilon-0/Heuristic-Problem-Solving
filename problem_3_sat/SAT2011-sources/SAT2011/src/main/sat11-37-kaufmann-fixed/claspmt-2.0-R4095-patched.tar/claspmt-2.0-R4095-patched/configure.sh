...
if [[ $mt == 0 ]]; then
	CXXFLAGS="${CXXFLAGS} -DDISABLE_MULTI_THREADING"
else
	TBB_INST=$TBB30_INSTALL_DIR
	if [ -n "$TBB_INST" ]; then
		if [ -z "$TBB_INCLUDE" ]; then
			TBB_INCLUDE="$TBB_INST/include"
		fi
		if [ -z "$TBB_LIB" ]; then
			TBB_LIB="$TBB_INST/lib"
		fi
	fi
	if [ -z "$TBB_INCLUDE" ]; then
		echo "*** Error: TBB include path not set!"
		echo "use '$0 TBB_INCLUDE=<path_to_tbb>/include'"
		exit 1
	fi
	if [ -z "$TBB_LIB" -o ! -f "$TBB_LIB/libtbb.so" ]; then
		echo "*** Error: TBB library path not set or '${TBB_LIB}/libtbb.so' not found!"
		echo "use '$0 TBB_LIB=<path_to_tbb_library>'"
		exit 1
	fi
	CXXFLAGS="${CXXFLAGS} -I ${TBB_INCLUDE}"
	LDFLAGS="${LDFLAGS} -ltbb -L ${TBB_LIB} -Xlinker \"--rpath=${TBB_LIB}\""
	BUILDPATH="${BUILDPATH}_mt"
fi

if [[ $claspre == 1 ]]; then
	CXXFLAGS="${CXXFLAGS} -DWITH_CLASPRE_SUPPORT"
fi
if [[ $static == 1 ]]; then
