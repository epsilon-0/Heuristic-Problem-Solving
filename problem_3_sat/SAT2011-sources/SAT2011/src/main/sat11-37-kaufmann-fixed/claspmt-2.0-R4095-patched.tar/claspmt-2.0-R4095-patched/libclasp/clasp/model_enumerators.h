...
private:
	...
	struct BTConstraint : SolveConstraint {
	...
	SolveConstraint* doInit(SharedContext& ctx, uint32 t, bool start);
	bool    isTrivial(bool disjoint) const { return disjoint; }
...
public:
	...
	~RecordEnumerator();
private:
	typedef mt::SharedLiterals SL;
	class  SolutionQueue : public MultiQueue<SL*, void (*)(SL*)> {
	public:
		typedef MultiQueue<SL*, void (*)(SL*)> base_type;
	...
	struct RecordConstraint : public SolveConstraint {
		RecordConstraint(SolutionQueue* q);
		bool   simplify(Solver& s, bool);
		void   destroy(Solver* s, bool detach);
		bool   add(Solver& s, LitVec& lits, const ClauseExtra& e);
		bool   integrateAll(Solver& s);
		bool   integrate(Solver& s, SL* lits, uint32 flags);
		uint32 flags() const {
	...
	SolveConstraint* doInit(SharedContext& ctx, uint32 t, bool start);
	bool   isTrivial(bool) const { return false; }
	bool   updateConstraint(Solver& s, bool disjoint);
	bool   doBacktrack(Solver& s, uint32 bl);
	uint32 getProjectLevel(Solver& s);
