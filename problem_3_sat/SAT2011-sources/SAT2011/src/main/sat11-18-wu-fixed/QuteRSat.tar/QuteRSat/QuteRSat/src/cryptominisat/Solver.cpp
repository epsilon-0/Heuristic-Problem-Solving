...
*/
lbool Solver::value      (const Var x) const       ///<The current value of a variable.
{ 
   return assigns[x];
}

lbool Solver::value      (const Lit p) const       ///<The current value of a literal.
{ 
                            ...
                            , const bool inOriginalInput)
{
    ...
    if (ps.size() == 0) {
        ...
        ok = (propagate().isNULL());
        //if( !ok ) std::cout << "Clause Prog UNSAT" << std::endl;
        return NULL;
    }
